...
export default async function MemberProfileViewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const session = await getServerSession(authOptions);
  const userRole = (session?.user as any)?.role;

  // Only STAFF, LIBRARIAN, and ADMIN can view this page
  if (!session?.user || userRole === "MEMBER") {
    redirect("/");
  }

  // Fetch user profile and loans using the UUID (id)
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <h2 className="text-3xl font-bold text-slate-800">Member Profile</h2>
        </div>
        {/* Only show Edit button if they have permission to edit this role */}
        {(userRole === 'ADMIN' || userRole === 'LIBRARIAN' || (userRole === 'STAFF' && user.role === 'MEMBER')) && (
          <Link href={`/members/edit/${user.id}`} className="bg-indigo-100 hover:bg-indigo-200 text-indigo-700 px-4 py-2 rounded-lg text-sm font-medium transition shadow-sm">
            Edit Member
          </Link>
        )}
      </div>
            ...
            <div className="divide-y divide-slate-100">
              {activeLoans.length === 0 ? (
                <div className="p-8 text-center text-slate-500">
                  <p>This member hasn't borrowed any books currently.</p>
                </div>
              ) : (
