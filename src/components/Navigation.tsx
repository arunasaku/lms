          ...
          <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto hide-scrollbar">
            {((session.user as any)?.role === 'ADMIN' || (session.user as any)?.role === 'LIBRARIAN' || (session.user as any)?.permDashboard) && (
              <>
                ...
                <Link onClick={closeMenu} href="/reports" className={`flex items-center px-3 py-2.5 rounded-lg group transition font-medium ${pathname?.startsWith('/reports') ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
